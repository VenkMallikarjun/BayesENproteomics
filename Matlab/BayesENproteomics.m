...
%       [peptides,proteins] required for a [protein,pathway] to be 
%       reported. Defaults to [3,5]. 
%
...
fid = fopen(exp_peps,'rt');
tLines = fgets(fid);
expCols = numel(strfind(tLines,',')) + 1;
fclose(fid);
exp_peps2 = readtable(exp_peps,'FileType','text','Delimiter',',','ReadVariableNames',false,'format',repmat('%q ',1,expCols));
exp_peps2 = table2cell(exp_peps2);

fid = fopen(norm_peps,'rt');
tLines = fgets(fid);
normCols = numel(strfind(tLines,',')) + 1;
if normCols ~= expCols
    error('normalisation and experimental datasets must have same number of columns.')
end
fclose(fid);
norm_peps2 = readtable(norm_peps,'FileType','text','Delimiter',',','ReadVariableNames',false,'format',repmat('%q ',1,normCols));
